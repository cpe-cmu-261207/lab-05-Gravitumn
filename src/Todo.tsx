    ...
    return (
        <div>
            <div className='mx-auto max-w-4xl'>
                ...
                {tasks.map(x=> <Task id={x.id} name={x.name} deleteFn={deleteTask} done = {finishTask} isFinish = {true}></Task>)}
                {finish.map(x=><Task id={x.id} name={x.name} deleteFn={deleteTask} done = {finishTask} isFinish = {false}></Task>)}
            </div>
        </div>
